...
public class ScrabbleTrie
{
	...
	public HashSet<String> generateWords(char [] tiles)
	{
	...
	{
		if (currentIndex != -1 && currentNode.getValidMarker(word))
		{
		...
		else
		{
			...
			for (int i = 0; i < tilesSize; i++)			//For each tile left in the tile rack...
			{
				currentTile = tiles.at(i);				//Get current tile.
				...
				if (n != null)							//If n != null, then recursively call the method to keep building the word.
				{
					...
					tiles.insert(i, currentTile);		//After recursive call, reset everything to previous state.
					word[currentIndex] = 0;
					currentIndex--;
				}
			}
